...
class ProfileLinksCommand extends Command
{
    ...
    */
    protected function execute(InputInterface $input, OutputInterface $output) : void
    {
        ...
        for($i = 0; $i < $totalRecords; $i++ ){
            ...
            $process = new Process("php application.php pl:vacuuming-2 --url='$url'");
            $process->start();
    ...
    */
    ...
    {
        try {
            $filter = $crawler->filter('.list-unstyled > .profile-cloud')->eq($k);
            return urldecode($filter->filter('h2 > a')->attr('href'));
        } catch (\Exception $e){
            return '';
        }
    }
    ...
    */
    ...
    {
        try{
            return (int)$crawler->filter('.list-unstyled > .profile-cloud ')->count();
        } catch (\Exception $e){
            return 0;
        }
    }
