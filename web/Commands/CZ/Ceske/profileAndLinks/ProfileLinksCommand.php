...
class ProfileLinksCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : void
    {
        ...
        for($i = 0; $i < $totalRecords; $i++ ){
            $url = $this->convertLink($crawler, $i);
                $process = new Process("php application.php cz:vacuuming-42 --url='$url'");
                $process->start();
    ...
    */
    ...
    {
        try {
            $filter = $crawler->filter('.fac-results > .fac-result')->eq($k)->filter('h2 > a')->attr('href');
    ...
    */
    ...
    {
        try{
            return (int)$crawler->filter('.fac-results > .fac-result')->count();
        } catch (\Exception $e){
            return 0;
        }
    }
