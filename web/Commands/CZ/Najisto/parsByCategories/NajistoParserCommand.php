...
class NajistoParserCommand extends Command
{
    ...
    */
    ...
    {
        $categories = file('web/Commands/CZ/Najisto/parsByCategories/list.txt', FILE_SKIP_EMPTY_LINES);
        $activeProcess = [];
        foreach($categories as $key => $category){
            try{
                for($i = 1; $i <= 99999; $i++){
                    $uri = $this->convertLink(trim($category), $i);
                    $process = new Process("php application.php cz:vacuuming-4 --url='$uri'");
                    $process->start();
                    $activeProcess[] = $process;
                    var_dump( "$key category is processed, now $i page is processed");
