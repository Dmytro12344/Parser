...
class PoslovnivodicParserCommand extends Command
{
    /**
     * Command config
    */
    ...
    */
    ...
    {
        $links = file('web/Commands/RS/Poslovnivodic/profileAndLinks/list.txt', FILE_SKIP_EMPTY_LINES);
        $activeProcess = [];
        foreach($links as $key => $link){
            try{
                ...
                for($i = 1; $i <= $totalPages; $i++){
                    ...
                    $process = new Process("php application.php rs:main-24 --url='$uri'");
                    $process->start();
    ...
    */
    public function getTotalPages($url) : int
    {
        try {
            ...
            $filter = $crawler->filter('.panel + .row > .hidden-xs')->text();
            $filter = explode('of', $filter);

            return (int)ceil((int)trim($filter[1]) / 10);
        } catch(\Exception $e){
            return 1;
        }
    }
